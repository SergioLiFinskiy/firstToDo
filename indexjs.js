...
function deleteTask(event) {
   if (event.target.dataset.action === "delete") {
    ...
    const name = parentNote.name; 
    tasks = tasks.filter((task) => task.id !== id);
    taskSortOfName = taskSortOfName.filter((task) => task.name !== id);
    taskSortUniq = taskSortUniq.filter((task) => task.name !== id);

    // clearPositionOfLocalStorage(name);
...
//     localStorage.removeItem('name', JSON.stringify(taskSortOfName));
